  ...
  convertDracoGeometryTo3JS(dracoDecoder, decoder, geometryType,
    ...
    if (geometryType == dracoDecoder.TRIANGULAR_MESH) {
      geometry.setIndex(new (geometryBuffer.indices.length > 65535
        ? THREE.Uint32BufferAttribute : THREE.Uint16BufferAttribute)(geometryBuffer.indices, 1));
    }
...
DRACOLoader.getDecoderModule = function () {
  const scope = this;
  const path = DRACOLoader.decoderPath;
  const config = DRACOLoader.decoderConfig;
